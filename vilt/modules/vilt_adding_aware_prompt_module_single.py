...
# 
        ...
        adding_text_prompt = torch.zeros(prompt_num, prompt_length, embed_dim)
        adding_text_prompt[:,2:3,:].fill_(1)            
        if self.learnt_p and self.prompt_type == 'attention':
            adding_text_prompt[:,prompt_length//2+2:prompt_length//2+3,:].fill_(1)
        self.adding_text_prompt = nn.Parameter(adding_text_prompt)

        adding_img_prompt = torch.zeros(prompt_num, prompt_length, embed_dim)
        adding_img_prompt[:,1:2,:].fill_(1)            
        if self.learnt_p and self.prompt_type == 'attention':
            adding_img_prompt[:,prompt_length//2+1:prompt_length//2+2,:].fill_(1)
        self.adding_img_prompt = nn.Parameter(adding_img_prompt)

        if not self.learnt_p:
            self.complete_prompt.requires_grad=False
            self.adding_text_prompt.requires_grad=False           
            self.adding_img_prompt.requires_grad=False

        # print(self.complete_prompt)
        # print(self.adding_img_prompt)
        # print(self.adding_text_prompt)
...
#                     image_embeds[idx,1:].fill_(1)
        ...
        for idx in range(len(img)):
            if batch["missing_type"][idx] == 0:
                ...
                prompt = self.adding_text_prompt
            elif batch["missing_type"][idx] == 2:
                prompt = self.adding_img_prompt
